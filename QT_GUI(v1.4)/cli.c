...
void derive_keys(const char* password, int aes_key_bits,
    ...
    pbkdf2_sha512((const uint8_t*)password, strlen(password),
                  salt, salt_len, 10000, kdf_output, 64);
...
static int encrypt_file_internal(const char* input_path, const char* output_path,
    ...
    // 파일 버퍼링 최적화 (모든 플랫폼)
    setvbuf(fin, NULL, _IOFBF, 512 * 1024);  // 512KB 버퍼
    ...
    // 파일 버퍼링 최적화 (모든 플랫폼)
    setvbuf(fout, NULL, _IOFBF, 512 * 1024);  // 512KB 버퍼
...
static int decrypt_file_internal(const char* input_path, const char* output_path,
    ...
    if (!ftemp) {
        fclose(fin);
        format_error_message(final_output_path, final_path_size, "Cannot create temporary file", 1);
        if (!progress_cb) printf("Error: Cannot create temporary file.\n");
        return 0;
    }
    
    // 임시 파일 버퍼링 최적화 (모든 플랫폼)
    setvbuf(ftemp, NULL, _IOFBF, 512 * 1024);  // 512KB 버퍼
    ...
    while (total_read < ciphertext_size) {
        ...
        if (bytes_read == 0) {
            ...
            if (total_read < ciphertext_size) {
                success = 0;
                if (!progress_cb) printf("\nError: Unexpected end of file. Expected %ld bytes, read %ld bytes.\n", 
    ...
    if (success && total_read != ciphertext_size) {
        success = 0;
        if (!progress_cb) printf("\nError: Incomplete decryption. Expected %ld bytes, read %ld bytes.\n", 
                                 ciphertext_size, total_read);
    }
...
#endif
        format_error_message(final_output_path, final_path_size, "Decryption failed before HMAC verification", 0);
        if (!progress_cb) printf("\nDecryption failed!\n");
        return 0;
    }
    ...
    if (memcmp(stored_hmac, computed_hmac, 64) != 0) {
        ...
        unlink(temp_file_path);
#endif
        format_error_message(final_output_path, final_path_size, "HMAC integrity verification failed", 0);
    ...
    if (ext_len > 0) {
        memcpy(format_ext, header.format, ext_len);
        format_ext[ext_len] = '\0';
    }
