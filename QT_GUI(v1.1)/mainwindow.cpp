...
void MainWindow::onEncrypt()
{
    ...
    if (fileList.isEmpty()) {
        QMessageBox::warning(this, "Error", "Please select at least one file to encrypt.");
        return;
    }
    ...
    // 출력 경로 업데이트
    for (int i = 0; i < fileList.size(); ++i) {
        if (fileList[i].outputPath.isEmpty()) {
            fileList[i].outputPath = generateDefaultOutputPath(fileList[i].inputPath, true);
        }
    }
    ...
    for (const FileInfo &fi : fileList) {
        filePairs.append(qMakePair(fi.inputPath, fi.outputPath));
    }
...
void MainWindow::onDecrypt()
{
    ...
    if (fileList.isEmpty()) {
        QMessageBox::warning(this, "Error", "Please select at least one encrypted file (.enc) to decrypt.");
        return;
    }
    ...
    // 출력 경로 업데이트 - 헤더에서 확장자를 읽어서 추가
    for (int i = 0; i < fileList.size(); ++i) {
        QString userPath = fileList[i].outputPath;
        QString autoPath = generateDefaultOutputPath(fileList[i].inputPath, false);
        ...
        if (userPath.isEmpty() || normalizedUserPath == normalizedAutoPath) {
            ...
            QFileInfo fileInfo(fileList[i].inputPath);
            QString baseName = fileInfo.completeBaseName();
            if (baseName.endsWith(".enc", Qt::CaseInsensitive)) {
                baseName = baseName.left(baseName.length() - 4);
            }
            ...
            // 헤더에서 확장자 읽기
            QString inputPathNative = QDir::toNativeSeparators(fileList[i].inputPath);
                ...
                if (fread(&header, 1, sizeof(header), fin) == sizeof(header)) {
                    ...
                    if (memcmp(header.signature, ENC_SIGNATURE, 4) == 0) {
                        ...
                        if (ext_len > 0) {
                            ...
                            fileList[i].outputPath = fileInfo.path() + QDir::separator() + baseName + extension;
                        } else {
                            // 확장자가 없으면 기본 경로 사용
                            fileList[i].outputPath = fileInfo.path() + QDir::separator() + baseName;
                        }
                    } else {
                        // 유효하지 않은 파일 형식
                        fileList[i].outputPath = fileInfo.path() + QDir::separator() + baseName;
