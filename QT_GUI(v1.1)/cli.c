...
static int decrypt_file_internal(const char* input_path, const char* output_path,
    ...
    if (fread(&header, 1, sizeof(header), fin) != sizeof(header)) {
        ...
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Cannot read file header.");
        }
    ...
    if (memcmp(header.signature, ENC_SIGNATURE, 4) != 0) {
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Invalid file format (not a .enc file).");
        }
    ...
    if (ciphertext_size <= 0) {
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Invalid file size.");
        }
    ...
    if (fread(stored_hmac, 1, 64, fin) != 64) {
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Cannot read HMAC.");
        }
    ...
    if (header.key_length_code == 0x01) aes_key_bits = 128;
        ...
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Unsupported AES key length.");
        }
    ...
    if (AES_set_key(&aes_ctx, aes_key, aes_key_bits) != CRYPTO_SUCCESS) {
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Failed to set AES key.");
        }
        return 0;
    }
    ...
    FILE* ftemp = platform_tmpfile();
    if (!ftemp) {
        fclose(fin);
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Cannot create temporary file.");
        }
        if (!progress_cb) printf("Error: Cannot create temporary file.\n");
        return 0;
    }
    ...
    while (total_read < ciphertext_size) {
        ...
        // 진행률 업데이트 - 콜백이 있으면 콜백, 없으면 print_progress
        if (progress_cb) {
            // 복호화 진행률: 0 ~ ciphertext_size (실제 읽은 바이트 수)
            progress_cb(total_read, ciphertext_size, user_data);
        } else {
            // 진행률 출력을 1% 단위로만 (성능 최적화)
    ...
    if (!success) {
        fclose(ftemp);
        platform_tmpfile_cleanup();  // Cleanup temporary file (Windows only)
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Decryption process failed during data decryption.");
        }
    ...
    if (memcmp(stored_hmac, computed_hmac, 64) != 0) {
        ...
        platform_tmpfile_cleanup();  // Cleanup temporary file (Windows only)
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: HMAC verification failed. Wrong password or corrupted file.");
        }
    ...
    if (!fout) {
        fclose(ftemp);
        platform_tmpfile_cleanup();  // Cleanup temporary file (Windows only)
        // 에러 정보를 final_output_path에 기록 (GUI에서 확인 가능)
        if (final_output_path && final_path_size > 0) {
            snprintf(final_output_path, final_path_size, "ERROR: Cannot create output file: %s", actual_output_path);
        }
        return 0;
    }
    ...
    while ((bytes_read = fread(buffer, 1, FILE_CHUNK_SIZE, ftemp)) > 0) {
        if (fwrite(buffer, 1, bytes_read, fout) != bytes_read) {
            ...
            if (final_output_path && final_path_size > 0) {
                snprintf(final_output_path, final_path_size, "ERROR: Failed to write output file: %s", actual_output_path);
            }
            return 0;
        }
        total_read += bytes_read;
