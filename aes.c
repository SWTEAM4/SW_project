...
// xtimes의 역함수 (GF(2^8)에서 곱셈 연산)
#define xtimes_3(input) (xtimes(input) ^ (input))  // 3·a = 2·a ^ a
#define xtimes_9(input) (xtimes(xtimes(xtimes(input)))) ^ input  // 9·a
#define xtimes_11(input) (xtimes(xtimes(xtimes(input)))) ^ xtimes(input) ^ input  // 11·a
#define xtimes_13(input) (xtimes(xtimes(xtimes(input)))) ^ xtimes(xtimes(input)) ^ input  // 13·a
#define xtimes_14(input) (xtimes(xtimes(xtimes(input)))) ^ xtimes(xtimes(input)) ^ xtimes(input)  // 14·a
 ...
 * * T-tables 최적화 버전으로 변경됨 (SubBytesAndMixColumns 사용)
 * @param state 4x4 크기의 데이터 블록
 * @note 이 함수는 더 이상 직접 호출되지 않으며, SubBytesAndMixColumns가 대신 사용됩니다.
 */
static void MixColumns(uint8_t state[4][4]) {
    // T-tables 최적화로 인해 이 함수는 사용되지 않음
    // 호환성을 위해 유지하지만 실제로는 SubBytesAndMixColumns를 사용합니다.
    SubBytesAndMixColumns(state);
}
...
int test_aes() {
    ...
    AES_CTR_crypt(&ctx, pt128, sizeof(pt128), ct128, iv128_enc); // 2. 암호화 수행

    print_hex("Key", key128, sizeof(key128));
    print_hex("Plaintext", pt128_copy, sizeof(pt128_copy));
    print_hex("IV", iv128_enc, 16);
    print_hex("Ciphertext", ct128, sizeof(ct128));
    ...
    // 복호화 검증
    printf("--- AES-128 CTR Decryption Test ---\n");
    ...
    AES_set_key(&ctx, key128, 128); // 키 재설정
    AES_CTR_crypt(&ctx, ct128, sizeof(ct128), decrypted128, iv128_dec); // 복호화 수행
    
    print_hex("Key", key128, sizeof(key128));
    print_hex("Ciphertext", ct128_copy, sizeof(ct128_copy));
    print_hex("IV", iv128_dec, 16);
    print_hex("Decrypted", decrypted128, sizeof(decrypted128));
    print_hex("Original PT", pt128_copy, sizeof(pt128_copy));
    printf("Decryption Test: %s\n\n", compare_hex(pt128_copy, decrypted128, sizeof(pt128_copy)) ? "SUCCESS" : "FAILURE");
    ...
    print_hex("Plaintext", pt192_copy, sizeof(pt192_copy));
    print_hex("IV", iv192_enc, 16);
    print_hex("Ciphertext", ct192, sizeof(ct192));
    print_hex("Expected", expected_ct192, sizeof(expected_ct192));
    printf("Encryption Test: %s\n\n", compare_hex(ct192, expected_ct192, sizeof(ct192)) ? "SUCCESS" : "FAILURE");
    
    // 복호화 검증
    printf("--- AES-192 CTR Decryption Test ---\n");
    ...
    AES_set_key(&ctx, key192, 192); // 키 재설정
    AES_CTR_crypt(&ctx, ct192, sizeof(ct192), decrypted192, iv192_dec); // 복호화 수행
    
    print_hex("Key", key192, sizeof(key192));
    print_hex("Ciphertext", ct192_copy, sizeof(ct192_copy));
    print_hex("IV", iv192_dec, 16);
    print_hex("Decrypted", decrypted192, sizeof(decrypted192));
    print_hex("Original PT", pt192_copy, sizeof(pt192_copy));
    printf("Decryption Test: %s\n\n", compare_hex(pt192_copy, decrypted192, sizeof(pt192_copy)) ? "SUCCESS" : "FAILURE");
    ...
    print_hex("Plaintext", pt256_copy, sizeof(pt256_copy));
    print_hex("IV", iv256_enc, 16);
    print_hex("Ciphertext", ct256, sizeof(ct256));
    print_hex("Expected", expected_ct256, sizeof(expected_ct256));
    printf("Encryption Test: %s\n\n", compare_hex(ct256, expected_ct256, sizeof(ct256)) ? "SUCCESS" : "FAILURE");
    
    // 복호화 검증
    printf("--- AES-256 CTR Decryption Test ---\n");
    ...
    AES_set_key(&ctx, key256, 256); // 키 재설정
    AES_CTR_crypt(&ctx, ct256, sizeof(ct256), decrypted256, iv256_dec); // 복호화 수행
    
    print_hex("Key", key256, sizeof(key256));
    print_hex("Ciphertext", ct256_copy, sizeof(ct256_copy));
    print_hex("IV", iv256_dec, 16);
    print_hex("Decrypted", decrypted256, sizeof(decrypted256));
    print_hex("Original PT", pt256_copy, sizeof(pt256_copy));
    printf("Decryption Test: %s\n\n", compare_hex(pt256_copy, decrypted256, sizeof(pt256_copy)) ? "SUCCESS" : "FAILURE");
