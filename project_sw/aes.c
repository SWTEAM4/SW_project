...
void AES128_dec(AES_STATE_t P, AES_STATE_t C, AES128_KEY_t K128)
{
    ...
    AddRoundKey(P, RoundKey[10]);

    //1~9 라운드
    for (int cnt_i = 9; cnt_i > 0; cnt_i--)
    {
        InvSubBytes(P);
        InvShiftRows(P);
        AddRoundKey(P, RoundKey[cnt_i]);
        InvMixColumns(P);
    }
...
{
    unsigned char RoundKey_0[24];
    unsigned char buf[13][16]; //라운드 키를 저장할 배열

    for (int i = 0; i < 24; i++)
    {
        RoundKey_0[i] = K192[i]; //MasterKey를 라운드 키 0으로 저장
    }
    ...
    for (int cnt_i = 0; cnt_i < 11; cnt_i++)
    {
        ...
        ShiftRows(C);
        MixColumns(C);
        AddRoundKey(C, buf[cnt_i + 1]);
    }

    //마지막 12 라운드
    SubBytes(C);
    ShiftRows(C);
    AddRoundKey(C, buf[12]);
}
...
void AES192_dec(AES_STATE_t P, AES_STATE_t C, AES192_KEY_t K192)
{
    ...
    AddRoundKey(P, buf[12]);

    //1~11 라운드
    for (int cnt_i = 11; cnt_i > 0; cnt_i--)
    {
        InvSubBytes(P);
        InvShiftRows(P);
        AddRoundKey(P, buf[cnt_i]);
        InvMixColumns(P);
    }
...
void AES256_enc(AES_STATE_t C, AES_STATE_t P, AES256_KEY_t K256)
{
    ...
    {
        RoundKey[0][i] = K256[i];
        RoundKey[1][i] = K256[i + 16];
        C[i] = P[i];
    }

    KeySchedule256(RoundKey);

    AddRoundKey(C, RoundKey[0]);

    //1~13 라운드
    for (int cnt_i = 0; cnt_i < 13; cnt_i++)
    {
        SubBytes(C);
        ShiftRows(C);
        MixColumns(C);
...
void AES256_dec(AES_STATE_t P, AES_STATE_t C, AES256_KEY_t K256)
{
    ...
    AddRoundKey(P, RoundKey[14]);

    //1~13 라운드
    for (int cnt_i = 13; cnt_i > 0; cnt_i--)
    {
        InvSubBytes(P);
        InvShiftRows(P);
        AddRoundKey(P, RoundKey[cnt_i]);
...
int main() {
    printf("=== AES 암호화 알고리즘 테스트 ===\n");
    printf("학번_이름: %s\n\n", SUBMISSION_INFO);
    
    // AES-128 테스트
    printf("--- AES-128 테스트 ---\n");
    AES_STATE_t plaintext = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 
                             0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};
    AES_STATE_t ciphertext;
    AES_STATE_t decrypted;
    AES128_KEY_t key128 = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 
                          0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
    
    print_hex("평문", plaintext, 16);
    print_hex("키", key128, 16);
    
    AES128_enc(ciphertext, plaintext, key128);
    print_hex("암호문", ciphertext, 16);
    
    AES128_dec(decrypted, ciphertext, key128);
    ...
    // AES-192 테스트
    printf("--- AES-192 테스트 ---\n");
    AES192_KEY_t key192 = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 
                          0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
                          0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17};
    
    print_hex("평문", plaintext, 16);
    print_hex("키", key192, 24);
    
    AES192_enc(ciphertext, plaintext, key192);
    print_hex("암호문", ciphertext, 16);
    
    AES192_dec(decrypted, ciphertext, key192);
    print_hex("복호문", decrypted, 16);
    ...
    printf("AES-192 복호화 검증: %s\n\n", match ? "성공" : "실패");
    
    // AES-256 테스트
    printf("--- AES-256 테스트 ---\n");
    AES256_KEY_t key256 = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 
                          0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
                          0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                          0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
    
    print_hex("평문", plaintext, 16);
    print_hex("키", key256, 32);
    
    AES256_enc(ciphertext, plaintext, key256);
    print_hex("암호문", ciphertext, 16);
    
    AES256_dec(decrypted, ciphertext, key256);
    print_hex("복호문", decrypted, 16);
